...
function pickType(index) {
    const types = ["temperature", "humidity", "power", "ups", "cooling"];
...
function randomDrift(range) {
    return (Math.random() - 0.5) * range * 2;
}
function isAnomaly() {
    return Math.random() < anomalyProbability;
}
function makeReading(device) {
    ...
    const quality = anomaly ? "bad" : Math.random() > 0.85 ? "uncertain" : "good";
    return {
...
function maybeToggleStatus(device) {
    if (Math.random() < 0.03) {
        device.status = device.status === "online" ? "offline" : "online";
    }
}
async function publishLocalBatch(readings) {
    if (readings.length === 0) {
        return;
    }
    await axios.post(`${backendUrl}/api/v1/readings/batch`, { readings }, {
        headers: {
...
function logState(message, meta) {
    if (meta) {
